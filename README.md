...
# Industrial PLC guard

Untrusted input is a setpoint; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a setpoint
- **Closed type:** `type Decision = SetParam(Param) | RejectParam` where `Param` is closed
- **Sink / capability:** `grant setParam confidence 75` → `privileged { setParam(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/plc-guard/demo.sh
```
...
- `plc-guard_safe.eide` — the correct design.
- `plc-guard_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
